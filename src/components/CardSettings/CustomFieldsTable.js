...
function CustomFieldsTable({fields, projects}) {
  ...
  const tableColumns = [
    ...
    { key: 'display', id: 'display', title: t('Display'), getValue: (item, column) =>
        (<Select hideSelected type="INLINE" selected={displayDataSet.find(data => data.key === item.presentation.id)} data={displayDataSet}/>)},
  ...
  return (
    <CustomFieldsTableContainer>
      ...
      <div>
        ...
        <FloatRightGreyInlineButton text>{t('Restore default')}</FloatRightGreyInlineButton>
      </div>
    </CustomFieldsTableContainer>
  );
